...
async function getTotalStudentsCountForRoom(client: Client, roomName: string): Promise<number> {
  const query = `
...
async function getRoomGender(client: Client, roomName: string): Promise<boolean | null> {
  const query = `
    SELECT gender
    FROM rooms
    WHERE name = $1;
